...
class Evaluation(object):
    ...
    def __init__(self,
        ...
        self.sim_seed = sim_seed if sim_seed is not None else np.random.randint(
            0, 1e6)
        ...
        self.run_directory = self.directory / \
            (run_directory or self.default_run_directory)
    ...
    def step(self):
        ...
        if self.step_callback_fn is not None:
            self.step_callback_fn(
                self.episode, self.wrapped_env, self.agent, transition, self.writer)

        # Record the experience.
        try:
            self.agent.record(previous_observation, action,
                              reward, self.observation, done, info)
    ...
    def run_batched_episodes(self):
        ...
        batch_sizes = near_split(
            self.num_episodes * episode_duration, size_bins=self.agent.config["batch_size"])
        self.agent.reset()
        for batch, batch_size in enumerate(batch_sizes):
            logger.info(
                "[BATCH={}/{}]---------------------------------------".format(batch+1, len(batch_sizes)))
            ...
            # Prepare workers
            env_config, agent_config = serialize(
                self.env), serialize(self.agent)
            cpu_processes = self.agent.config["processes"] or os.cpu_count()
            workers_sample_counts = near_split(batch_size, cpu_processes)
            workers_starts = list(np.cumsum(
                np.insert(workers_sample_counts[:-1], 0, 0)) + np.sum(batch_sizes[:batch]))
            ...
            if cpu_processes == 1:
                ...
                with Pool(processes=cpu_processes) as pool:
                    results = pool.starmap(
                        Evaluation.collect_samples, workers_params)
            trajectories = [
                trajectory for worker in results for trajectory in worker]

            # Fill memory
            for trajectory in trajectories:
                # Check whether the episode was properly finished before logging
                if trajectory[-1].terminal:
                    self.after_all_episodes(
                        episode, [transition.reward for transition in trajectory])
                episode += 1
    ...
    def save_agent_model(self, identifier, do_save=True):
        ...
        episode_path = None
        if do_save:
            episode_path = Path(self.run_directory) / \
                "checkpoint-{}.tar".format(identifier)
            try:
    ...
    def load_agent_model(self, model_path):
        ...
        try:
            ...
            if model_path:
                logger.info("Loaded {} model from {}".format(
    ...
    def after_all_episodes(self, episode, rewards, duration):
        ...
        self.writer.add_scalar(
            'episode/return', sum(r*gamma**t for t, r in enumerate(rewards)), episode)
        self.writer.add_scalar(
            'episode/fps', len(rewards) / max(duration, 1e-6), episode)
    ...
    def after_some_episodes(self, episode, rewards,
        ...
        if self.training:
            ...
            self.filtered_agent_stats += 1 / episodes_window * \
                (np.sum(rewards) - self.filtered_agent_stats)
    ...
    def write_logging(self):
        ...
        rl_agents.trainer.logger.add_file_handler(
            self.run_directory / self.LOGGING_FILE.format(file_infix))
...
agent_config = {
    "model": {
        "type": "DuelingNetwork",  # Dueling architecture
        "layers": [256, 256]
    },
...
agent.load("highway_ddqn/saved_models/ddqn_reward_change.tar")
agent.eval()
...
evaluation = Evaluation(
    env=env,
    agent=agent,
    directory="highway_ddqn/",
    num_episodes=200,  # Testing episodes, can be fewer than training
    training=False,
    display_env=True  # Set to True if you want to render during testing
)

# Run the evaluation (testing) episodes
# evaluation.test()
...
for episode in range(num_test_episodes):
    ...
    while not (done or truncated):  # End loop if either 'done' or 'truncated' is True
        ...
        env.render()
        time.sleep(0.05)  # Adjust sleep time to control the speed of rendering
