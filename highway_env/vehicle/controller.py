...
class ControlledVehicle(Vehicle):
    ...
    def steering_control(self, target_lane_index: LaneIndex) -> float:
        ...
        slip_angle = np.arcsin(
            np.clip(
                self.LENGTH / 2 /
                utils.not_zero(self.speed) * heading_rate_command,
                -1,
                1,
            )
        )
...
class MDPVehicle(ControlledVehicle):
    ...
    def distance_to_front_car(self):
        """
        Calculate the distance to the nearest car directly in front of the agent's vehicle in the same lane.

        Returns:
            float: Distance to the closest front vehicle in the same lane, or infinity if no vehicle is ahead.
        """
        front_distances = [
            vehicle.position[0] - self.position[0]
            for vehicle in self.road.vehicles
            if vehicle.lane_index == self.lane_index and vehicle.position[0] > self.position[0]
        ]
        # Return the minimum positive distance or infinity if no vehicles are ahead
